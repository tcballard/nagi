...
pub fn find(
    ...
    let mut add = |title: &str, url: &str, kind: Kind| {
        ...
        if results.len() < 6
            && url != "about:blank"
            && query.split_whitespace().all(|q| text.contains(q))
    ...
    for (id, page, is_private) in tabs {
        if *is_private == private {
            add(&page.title, &page.url, Kind::Tab(*id));
        }
    }
    for page in &state.bookmarks {
        add(&page.title, &page.url, Kind::Bookmark);
    }
    if !private {
        for visit in &state.history {
            add(&visit.title, &visit.url, Kind::History);
        }
    }
    results
}
...
mod tests {
    ...
    fn suggestions_are_local_deduplicated_and_private() {
        let page = Page {
            title: "Quiet sea".into(),
            url: "https://sea.test".into(),
            pinned: false,
        };
        let secret = Page {
            title: "Quiet secret".into(),
            url: "https://secret.test".into(),
            pinned: false,
        };
    ...
    fn old_state_restores_default_geometry_and_bounds_invalid_sizes() {
        ...
        let state: State =
            serde_json::from_str(r#"{"window":{"width":-1,"height":99999}}"#).unwrap();
